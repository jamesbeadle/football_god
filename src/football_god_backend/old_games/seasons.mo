...
module {
  ...
  public class Seasons() {
    ...
    public func getSeason(seasonId : Nat16) : ?Types.Season {
      let foundSeason = List.find<Types.Season>(
        seasons,
        func(season : Types.Season) : Bool {
    ...
    public func getGameweek(seasonId : Nat16, gameweekNumber : Nat8) : ?Types.Gameweek {
      ...
      switch (foundSeason) {
        case (null) { return null };
        case (?season) {
          let foundGameweek = List.find<Types.Gameweek>(
            season.gameweeks,
            func(gameweek : Types.Gameweek) : Bool {
    ...
    public func checkValidPredictions(seasonId : Nat16, gameweekNumber : Nat8, predictions : [DTOs.FixtureDTO]) : Bool {
      ...
      switch (fixtures) {
        ...
        case (?f) {
          ...
          let fixturesWithPredictions = Array.filter<Types.Fixture>(
            List.toArray(f),
            func(fixture : Types.Fixture) : Bool {
              return Array.find<DTOs.FixtureDTO>(
                predictions,
                func(prediction : DTOs.FixtureDTO) : Bool {
    ...
    public func getSeasons() : [Types.Season] {
      return List.toArray(
        List.map<Types.Season, Types.Season>(
          seasons,
    ...
    public func getGameweeks(seasonId : Nat16) : [Types.Gameweek] {
      ...
      switch (foundSeason) {
        ...
        case (?season) {
          return List.toArray(
            List.map<Types.Gameweek, Types.Gameweek>(
              season.gameweeks,
              func(gameweek : Types.Gameweek) : Types.Gameweek {
                return {
                  number = gameweek.number;
                  status = gameweek.status;
                  totalPot = gameweek.totalPot;
    ...
    public func updateSeason(id : Nat16, newName : Text, newYear : Nat16) : Result.Result<(), Types.Error> {
      ...
      seasons := List.map<Types.Season, Types.Season>(
        seasons,
        func(season : Types.Season) : Types.Season {
          if (season.id == id) {
            {
        ...
        func(season : Types.Season) : Types.Season {
          if (season.id == seasonId) {
            ...
            return {
              ...
              name = season.name;
              year = season.year;
              gameweeks = List.map<Types.Gameweek, Types.Gameweek>(
                season.gameweeks,
                func(gameweek : Types.Gameweek) : Types.Gameweek {
      ...
      switch (foundSeason) {
        ...
        case (?season) {
          ...
          switch (foundGameweek) {
            case (null) { return null };
            case (?gameweek) {
              let foundFixture = List.find<Types.Fixture>(
                gameweek.fixtures,
                func(fixture : Types.Fixture) : Bool {
                ...
                func(gameweek : Types.Gameweek) : Types.Gameweek {
                  if (gameweek.number == gameweekNumber) {
                    return {
                      ...
                      winningShare = gameweek.winningShare;
                      fixtureCount = 0;
                      fixtures = List.map<Types.Fixture, Types.Fixture>(
                        gameweek.fixtures,
                        func(fixture : Types.Fixture) : Types.Fixture {
                ...
                func(gameweek : Types.Gameweek) : Types.Gameweek {
                  if (gameweek.number == gameweekNumber) {
                    return {
                      ...
                      totalPot = gameweek.totalPot;
                      winningShare = gameweek.winningShare;
                      fixtures = List.filter<Types.Fixture>(
                        gameweek.fixtures,
                        func(fixture : Types.Fixture) : Bool {
                          if (fixture.id == fixtureId) {
                            return false;
