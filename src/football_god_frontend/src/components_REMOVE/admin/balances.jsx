...
const Balances = () => {
  ...
  const fetchViewData = async () => {
    ...
    const data = await football_god_backend_actor.getUserBalancesDTO(
      Number(page),
      Number(count)
    );
    setViewData(data);
        ...
        <Row className="justify-content-md-center">
          <Col md={10}>
            <Card className="mt-4 custom-card mb-4">
              ...
              <Card.Body>
                <p>
                  Sweepstake Pot Balance:{" "}
                  {(Number(viewData.potAccountBalance) / 1e8).toFixed(4)} ICP
                </p>
                ...
                <div className="d-flex justify-content-center mt-3 mb-3">
                  <ButtonGroup>
                    ...
                    <Button
                      onClick={() => handlePageChange(1)}
                      className="custom-button"
                      disabled={page * count >= viewData.totalEntries}
                    >
                      Next
                    </Button>
                  </ButtonGroup>
