...
const CorrectPredictions = () => {
  ...
  const fetchViewData = async () => {
    ...
    const data = await football_god_backend_actor.getCorrectPredictionsDTO(
      Number(seasonId),
          ...
          <Col md={12}>
            <Card className="mt-4 custom-card mb-4">
              ...
              <Card.Body>
                ...
                <p className="mt-3">
                  <strong>Fixture:</strong> {viewData.homeTeamName} v{" "}
                  {viewData.awayTeamName}
                </p>
                <p className="mt-3">
                  <strong>Result:</strong> {viewData.homeTeamGoals} -{" "}
                  {viewData.awayTeamGoals}
                </p>
                    ...
                    <tbody>
                      {viewData.predictions.map((submission) => (
                        <tr key={submission.principalName}>
                          ...
                          <td>
                            <LinkContainer
                              to={`/view-prediction/${submission.principalName}/${viewData.seasonId}/${viewData.gameweekNumber}`}
                            >
