...
const Leaderboard = () => {
  ...
  const fetchViewData = async () => {
    const data = await football_god_backend_actor.getLeaderboardDTO(
      Number(selectedSeason),
      Number(selectedGameweek),
      Number(page),
      Number(count)
    );
    setViewData(data);
  ...
  const handleViewPrediction = (userId) => {
    ...
    navigate(
      `/view-prediction/${userId}/${Number(seasonId)}/${Number(gameweekNumber)}`
    );
  };
          ...
          <Col md={8}>
            ...
            <Dropdown className="mt-3">
              ...
              <Dropdown.Menu>
                {Array.from({ length: 38 }, (_, i) => i + 1).map((number) => (
                  <Dropdown.Item
                    key={number}
                    onClick={() => setSelectedGameweek(number)}
                  >
                    Gameweek {number}
                  </Dropdown.Item>
                ))}
            ...
            {viewData.status > 1 && (
              <p className="mt-3">
                ...
                {(Number(viewData.totalPot) / 1e8).toFixed(2)} ICP
              </p>
            )}
            {viewData.status == 3 && (
              <p className="mt-3">
                <strong>Winners Share:</strong>{" "}
                {(Number(viewData.winningShare) / 1e8).toFixed(2)} ICP
              </p>
            )}
            <p className="mt-3">
              <small>
                You can update your display name in on your profile page.
              </small>
            </p>
              ...
              <tbody>
                {viewData.leaderboardEntries.map((entry) => (
                  <tr key={entry.principalName}>
                    ...
                    <td>
                      {entry.principalName == entry.displayName
                        ? "Unknown"
                        : entry.displayName}
                    </td>
                    <td>
                    ...
                    <td>
                      <Button
                        ...
                        onClick={() =>
                          handleViewPrediction(entry.principalName)
                        }
                      >
                        View
                      </Button>
            ...
            <div className="d-flex justify-content-center mt-3 mb-3">
              <ButtonGroup>
                ...
                <Button
                  className="custom-button"
                  onClick={() => handlePageChange(1)}
                  disabled={(page + 1) * count >= viewData.totalEntries}
                >
                  Next
                </Button>
              </ButtonGroup>
