...
const ViewPrediction = () => {
  ...
  const fetchViewData = async () => {
    const data = await football_god_backend_actor.getViewPredictionDTO(
      userId,
      Number(seasonId),
          ...
          <Col md={8}>
            <Card className="mt-4 custom-card mb-4">
              ...
              <Card.Body>
                ...
                <br />
                <small>
                  Total Correct: {viewData.correctScores} /{" "}
                  {viewData.totalFixtures}
                </small>
                <br />
                <br />
                <ListGroup>
                  {viewData.fixtures.map((prediction) => {
                    ...
                    return (
                      <ListGroup.Item
                        ...
                        className={`prediction-${cssClass} mt-2`}
                      >
                        <div>
                          {prediction.homeTeamName} vs {prediction.awayTeamName}
                        </div>
                        <div>
                          Prediction: {prediction.homeTeamPrediction} -{" "}
                          {prediction.awayTeamPrediction}
                        </div>
                        {prediction.status > 0 && (
                          <div>
                            Actual Score: {prediction.homeTeamGoals} -{" "}
