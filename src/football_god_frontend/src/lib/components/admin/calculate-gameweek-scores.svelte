<script lang="ts">
    ...
    async function calculateGameweekScores() {
      busyStore.startBusy({
        initiator: "calculate-gameweek-scores",
        text: "Calculate Gameweek Scores...",
      });
      try {
        ...
        toastsError({
          msg: { text: "Error calculating gameweek scores." },
          err: error,
        });
        console.error("Error calculating gameweek score:", error);
        closeModal();
      } finally {
        busyStore.stopBusy("calculate-gameweek-scores");
      }
    }
