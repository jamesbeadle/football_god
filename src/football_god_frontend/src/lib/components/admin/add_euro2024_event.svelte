<script lang="ts">
  ...
  $: filteredFixtures = fixtures.filter(fixture => {
      const selectedStageKey = Object.keys(selectedStage)[0];
      const fixtureStageKey = Object.keys(fixture.stage)[0];
      return selectedStageKey === fixtureStageKey;
  });

  $: {
      const selectedFixture = fixtures.find(fixture => fixture.id === fixtureId);
      filteredTeams = selectedFixture
        ? teams.filter(team => team.id === selectedFixture.homeTeamId || team.id === selectedFixture.awayTeamId)
        : [];
  }
  ...
  function validateForm() {
      if (selectedStage && fixtureId && teamId && selectedEventType) {
          if (isPlayerRequired(selectedEventType) && !playerId) {
              alert('Player selection is required for the selected event type.');
              return false;
          }
          return true;
      }
  ...
  {:else}
      <div class="mx-4 p-4">
          ...
          <form on:submit|preventDefault={handleSubmit}>
              ...
              <div class="mt-4">
                  <label for="selected-fixture" class="block mb-2">Fixture</label>
                  <select name="selected-fixture" bind:value={fixtureId} class="block w-full p-2 border bg-gray-500 rounded">
                      {#each filteredFixtures as fixture}
                          {@const homeTeam = teams.find(x => x.id == fixture.homeTeamId)}
                          {@const awayTeam = teams.find(x => x.id == fixture.awayTeamId)}
                          {#if homeTeam && awayTeam}
                              <option value={fixture.id}>{homeTeam.name} v {awayTeam.name}</option>
                          {/if}
                      {/each}
                  </select>
              </div>
              ...
              {#if isPlayerRequired(selectedEventType)}
              <div class="mt-4">
