<script lang="ts">
    ...
    async function snapshotFantasyTeams() {
      busyStore.startBusy({
        initiator: "snapshot-fantasy-teams",
        text: "Snapshotting fantasy teams...",
      });
      try {
        ...
        await adminStore.snapshotManagers(applicationName);

        await closeModal();
        toastsShow({
          text: "Fantasy teams snapshot taken.",
          level: "success",
          duration: 2000,
        });
      } catch (error) {
        ...
        busyStore.stopBusy("snapshot-fantasy-teams");
      }
    }
